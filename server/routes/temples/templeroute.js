...
const createTemple = express_1.default.Router();
...
createTemple.post('/createTemple', upload.fields([
    ...
    try {
        ...
        const temple_thumbnail = ((_a = req.files) === null || _a === void 0 ? void 0 : _a.temple_thumbnail) ? req.files.temple_thumbnail[0].location : null;
        const uploadedFiles = ((_b = req.files) === null || _b === void 0 ? void 0 : _b.temple_media) || [];
        ...
        const temple = yield TemplesModel_1.default.create({
            temple_name,
            temple_location,
            temple_description,
            phone_number,
        ...
        if (image_urls.length > 0 || video_urls.length > 0) {
            yield TempleImagesModel_1.default.create({
...
createTemple.put('/updateTemple/:id', upload.fields([
    ...
    try {
        ...
        const temple = yield TemplesModel_1.default.findOne({ where: { temple_id: req.params.id } });
        if (!temple) {
            return res.status(404).json({ message: 'Temple not found' });
        }
        // Process uploaded files correctly
        const temple_thumbnail = ((_a = req.files) === null || _a === void 0 ? void 0 : _a.temple_thumbnail) ? req.files.temple_thumbnail[0].location : temple.temple_thumbnail;
        const uploadedFiles = ((_b = req.files) === null || _b === void 0 ? void 0 : _b.temple_media) || [];
        // Store all images and videos as an array
        const image_urls = uploadedFiles.filter(file => file.mimetype.startsWith('image/')).map(file => file.location);
        const video_urls = uploadedFiles.filter(file => file.mimetype.startsWith('video/')).map(file => file.location);
        ...
        if (image_urls.length > 0 || video_urls.length > 0) {
            ...
            yield TempleImagesModel_1.default.upsert({
                temple_id: temple.temple_id, // Foreign key to TemplesModel
...
createTemple.put('/addTempleMedia/:temple_id', upload.array('temple_media', 10), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        console.log('Files received:', req.files);
        console.log('Temple ID:', req.params.temple_id);
        const { temple_id } = req.params;
        //  Check if temple exists
        let existingRecord = yield TempleImagesModel_1.default.findOne({ where: { temple_id } });
        if (!existingRecord) {
            existingRecord = yield TempleImagesModel_1.default.create({
                temple_id: temple_id,
                image_urls: [],
                video_urls: [],
            });
        }
        const newImageUrls = req.files.filter((file) => file.mimetype.startsWith('image/')).map((file) => file.location);
        const newVideoUrls = req.files.filter((file) => file.mimetype.startsWith('video/')).map((file) => file.location);
...
createTemple.put('/updateTempleThumbnail/:temple_id', upload.single('temple_thumbnail'), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { temple_id } = req.params;
        // Check if temple exists
        ...
        const temple_thumbnail = req.file ? req.file.location : temple.temple_thumbnail;
        // Update temple with new thumbnail URL
        yield temple.update({ temple_thumbnail });
