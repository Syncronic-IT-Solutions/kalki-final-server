...
import CouponModel from '../../db/models/coupons/CouponModel';
import UserCouponUsageModel from '../../db/models/coupons/CouponUsageHistoryModel'; // Adjust import path
...
couponRouter.post('/coupon-create', async (req: any, res: any) => {
  try {
    ...
    const existingCoupon = await CouponModel.findOne({ where: { coupon_code } });
    if (existingCoupon) {
      return res.status(400).json({
        success: false,
...
couponRouter.get('/coupon/:id', async (req: any, res: any) => {
  ...
  try {
    ...
    const coupon = await CouponModel.findOne({ where: { coupon_id: couponId } });
...
couponRouter.post('/coupon-redeem', async (req: any, res: any) => {
  ...
  try {
    ...
    const coupon = await CouponModel.findOne({ where: { coupon_code } });
    ...
    const userCouponUsage = await UserCouponUsageModel.findOne({
      where: {
        userid,
    ...
    if (userCouponUsage) {
      await userCouponUsage.update({ usage_count: userCouponUsage.usage_count + 1 });
    } else {
      await UserCouponUsageModel.create({
        userid,
...
couponRouter.put('/update-coupon/:id', async (req: any, res: any) => {
  const couponId = req.params.id;
  const {
    coupon_code,
    description,
...
couponRouter.post('/user/coupon/usage', async (req: any, res: any) => {
  ...
  try {
    ...
    const existingUsage = await UserCouponUsageModel.findOne({
    ...
    if (existingUsage) {
      ...
      const newUsage = await UserCouponUsageModel.create({
        userid,
        coupon_id,
...
couponRouter.get('/user/coupon/usage/:userid/:coupon_id', async (req: any, res: any) => {
  const { userid, coupon_id } = req.params;

  try {
    const usage = await UserCouponUsageModel.findOne({
      where: { userid, coupon_id },
    });
...
couponRouter.get('/user/coupon/usages', async (req: any, res: any) => {
  try {
    const usages = await UserCouponUsageModel.findAll();
